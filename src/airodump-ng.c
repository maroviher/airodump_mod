...
void DoAfterWpaHandshakeCaptured(struct AP_info *ap_cur, struct ST_info *st_cur)
{
	LogWpaHandshake(ap_cur, st_cur);
	fclose(pFileWpaHandsh);
	pFileWpaHandsh = 0;
	//char* pStrHandshakeFile = GetPathToHandshakeFile(ap_cur);
	char fName[1111];
	snprintf(fName, sizeof(fName), "%sessid.txt", strWpaHandshDir);
	...
	size_t len = strlen((char*)ap_cur->essid);
	if(len==fwrite(ap_cur->essid, len, 1, f))
	{
		fprintf( stderr, "Could not write \"%s\".\n", "essid.txt");
		exit(1);
	}
	fclose(f);
	//chmod(pStrHandshakeFile, S_IROTH);
}

int CreateWpaHandshakeFile(struct AP_info *ap_cur)
{
	if(pFileWpaHandsh)
		return -2;
	...
	if (fwrite(&pfh, 1, sizeof(pfh), pFileWpaHandsh) != (size_t) sizeof(pfh))
	{
...
int WriteWPAHSpacket(unsigned char *h80211, int caplen, struct rx_info *ri)
{
	if(!pFileWpaHandsh || caplen < 10)
		return -1;
	...
	if (fwrite(&pkh, 1, sizeof(pkh), pFileWpaHandsh) != (size_t) sizeof(pkh))
	{
		perror("fwrite(packet header) failed");
		return (1);
	}

	if (fwrite(h80211, 1, pkh.caplen, pFileWpaHandsh) != (size_t) pkh.caplen)
	{
...
int RestartAirbase(struct AP_info *ap_cur)
{
	...
	if(0 != system(buf))
	{
		printf("startAttack.sh error");
		//sighandler(SIGINT);//restore terminal and exit
	}
	return 0;
}
	...
	if (buffer != NULL && strlen(buffer) > 0)
	{
		...
		if (buffer_manuf != NULL)
		{
			...
			if (*buffer_manuf != '\0')
			{
				...
				if (*buffer_manuf != '\0')
				{
					if ((manuf = (char *) malloc(
							(strlen(buffer_manuf) + 1) * sizeof(char))) == NULL)
					{
						perror("malloc failed");
						return NULL;
					}
					snprintf(manuf, strlen(buffer_manuf) + 1, "%s",
							buffer_manuf);
				}
			}
		}
...
void ChannelHopper()
{
	...
	if (time_since_last_switch >= (unsigned int)G.hopfreq*1000)
	{
		if (0 == wi_set_channel(wi[0], bg_chans[i_ChanRunner]))
		{
			time_last_channel_switch = cur_time;
			G.channel[0] = bg_chans[i_ChanRunner];
			if (i_ChanRunner < (sizeof(bg_chans) / sizeof(bg_chans[0]) - 2))
				i_ChanRunner++;
			else
				i_ChanRunner = 0;
		}
...
struct oui * load_oui_file(void)
{
	...
	while (fgets(buffer, sizeof(buffer), fp) != NULL)
	{
		...
		if (sscanf(buffer, "%2c-%2c-%2c", a, b, c) == 3)
		{
			...
			snprintf(oui_ptr->id, sizeof(oui_ptr->id), "%c%c:%c%c:%c%c", a[0],
					a[1], b[0], b[1], c[0], c[1]);
...
int check_shared_key(unsigned char *h80211, int caplen)
{
	...
	if ((unsigned) caplen > sizeof(G.sharedkey[0]))
		return 1;
	...
	if (textlen + 4 != G.sk_len2)
	{
		snprintf(G.message, sizeof(G.message),
				...
				*(G.sharedkey[0] + m_bmac), *(G.sharedkey[0] + m_bmac + 1),
				*(G.sharedkey[0] + m_bmac + 2), *(G.sharedkey[0] + m_bmac + 3),
				*(G.sharedkey[0] + m_bmac + 4), *(G.sharedkey[0] + m_bmac + 5));
		return 1;
	}

	if ((unsigned) textlen > sizeof(text) - 4)
		return 1;
	...
	snprintf(ofn, sizeof(ofn) - 1, "%s-%02d-%02X-%02X-%02X-%02X-%02X-%02X.%s",
			G.prefix, G.f_index, *(G.sharedkey[0] + m_bmac),
			*(G.sharedkey[0] + m_bmac + 1), *(G.sharedkey[0] + m_bmac + 2),
			*(G.sharedkey[0] + m_bmac + 3), *(G.sharedkey[0] + m_bmac + 4),
			*(G.sharedkey[0] + m_bmac + 5), "xor");
	...
	snprintf(G.message, sizeof(G.message),
			...
			textlen + 4, *(G.sharedkey[0] + m_bmac),
			*(G.sharedkey[0] + m_bmac + 1), *(G.sharedkey[0] + m_bmac + 2),
			*(G.sharedkey[0] + m_bmac + 3), *(G.sharedkey[0] + m_bmac + 4),
...
void update_rx_quality()
{
	...
	while (ap_cur != NULL)
	{
				...
				|| time_diff > (QLT_TIME * 1000000))
		{
			...
			{
				capt_time = (1000000
						* (ap_cur->ftimel.tv_sec - ap_cur->ftimef.tv_sec) //time between first and last captured frame
				+ (ap_cur->ftimel.tv_usec - ap_cur->ftimef.tv_usec));

				miss_time = (1000000
						* (ap_cur->ftimef.tv_sec - ap_cur->ftimer.tv_sec) //time between timer reset and first frame
				...
				{
					missed_frames = ((float) ((float) miss_time
							/ (float) capt_time)
...
int dump_initialize(char *prefix, int ivs_only)
{
	...
	{
		memset(ofn, 0, ofn_len);
		snprintf(ofn, ofn_len, "%s-%02d.%s", prefix, G.f_index,
				AIRODUMP_NG_CSV_EXT);
...
int list_check_decloak(struct pkt_buf **list, int length, unsigned char* packet)
{
	...
	timediff = (((tv1.tv_sec - ((*list)->ctime.tv_sec)) * 1000000)
			+ (tv1.tv_usec - ((*list)->ctime.tv_usec))) / 1000;
	...
	while (next != NULL)
	{
		if (next->next != NULL)
		{
			timediff = (((tv1.tv_sec - (next->next->ctime.tv_sec)) * 1000000)
					+ (tv1.tv_usec - (next->next->ctime.tv_usec))) / 1000;
			if (timediff > BUFFER_TIME)
			{
		...
		int cardnum)
{
	...
	if (ap_cur == NULL)
	{
		...
		{
			ap_cur->manuf = get_manufacturer(ap_cur->bssid[0], ap_cur->bssid[1],
					ap_cur->bssid[2]);
		}
...
//        printf("initial auth %d\n", ap_cur->wpa_state);
	...
	if (st_cur == NULL)
	{
		...
		{
			st_cur->manuf = get_manufacturer(st_cur->stmac[0], st_cur->stmac[1],
					st_cur->stmac[2]);
		}
...
//            ap_cur->encryption = 2 + ( ( h80211[z + 3] & 0x20 ) >> 5 );
			...
			if (!uniqueiv_check(ap_cur->uiv_root, &h80211[z]))
			{
				...
				if (G.f_ivs != NULL)
				{
					...
					else
					{
						...
						num_xor = known_clear(clear + 2, &clen, weight, h80211,
								dlen);
						ivs2.flags |= IVS2_PTW;
						...
						memcpy(clear + 4 + 1 + 1 + 32 * num_xor, weight,
								16 * sizeof(int));
						//clear is now the keystream
					}
				...
				&& (h80211[1] & 0x40) != 0x40)
		{
					...
					&& (h80211[z + 5] & 0x01) != 0)
			{
				...
				if ((st_cur->wpa.state & 4) != 4)
				{
					...
					memcpy(st_cur->wpa.eapol, &h80211[z],
							st_cur->wpa.eapol_size);
					...
					&& (h80211[z + 5] & 0x01) != 0)
			{
				...
				if ((st_cur->wpa.state & 4) != 4)
				{
					...
					if (caplen - (uint) z < st_cur->wpa.eapol_size
							|| st_cur->wpa.eapol_size == 0
							|| caplen - (uint) z < 81 + 16
							|| st_cur->wpa.eapol_size
									> sizeof(st_cur->wpa.eapol))
			...
			if (st_cur->wpa.state == 7)
			{
				...
				if (G.f_ivs != NULL)
				{
					...
					if (fwrite(&(st_cur->wpa), 1, sizeof(struct WPA_hdsk),
							G.f_ivs) != (size_t) sizeof(struct WPA_hdsk))
					{
	...
	WriteWPAHSpacket(h80211, caplen, ri);

	if(pFileWpaHandsh && ap_cur && st_cur && ap_cur->bWPAHS_completed && ap_cur->m_bUnderAttack)
	{
		DoAfterWpaHandshakeCaptured(ap_cur, st_cur);
	}
			...
			while (ap_cur != NULL)
			{
				...
				case SORT_BY_ENC:
					if (((ap_cur->security & STD_FIELD)
							- (ap_min->security & STD_FIELD)) * G.sort_inv < 0)
						ap_min = ap_cur;
					break;
				case SORT_BY_CIPHER:
					if (((ap_cur->security & ENC_FIELD)
							- (ap_min->security & ENC_FIELD)) * G.sort_inv < 0)
						ap_min = ap_cur;
					break;
				case SORT_BY_AUTH:
					if (((ap_cur->security & AUTH_FIELD)
							- (ap_min->security & AUTH_FIELD)) * G.sort_inv < 0)
						ap_min = ap_cur;
					break;
				case SORT_BY_ESSID:
					if ((strncasecmp((char*) ap_cur->essid,
							(char*) ap_min->essid, MAX_IE_ELEMENT_SIZE))
							* G.sort_inv < 0)
						ap_min = ap_cur;
					break;
...
char * getStringTimeFromSec(double seconds)
{
	...
	if (hour[2] != 0)
		snprintf(HourTime, 128, "%d %s", hour[2],
				(hour[2] == 1) ? "hour" : "hours");
	if (hour[1] != 0)
		snprintf(MinTime, 128, "%d %s", hour[1],
				(hour[1] == 1) ? "min" : "mins");
...
//        G.maxaps--;
	...
	if (G.gps_loc[0])
	{
		snprintf(buffer, sizeof(buffer) - 1, " %s[ GPS %8.3f %8.3f %8.3f %6.2f "
				"][ Elapsed: %s ][ %04d-%02d-%02d %02d:%02d ", G.batt,
				G.gps_loc[0], G.gps_loc[1], G.gps_loc[2], G.gps_loc[3],
				G.elapsed_time, 1900 + lt->tm_year, 1 + lt->tm_mon, lt->tm_mday,
				lt->tm_hour, lt->tm_min);
	}
	else
	{
		snprintf(buffer, sizeof(buffer) - 1,
				" %s[ Elapsed: %s ][ %04d-%02d-%02d %02d:%02d ", G.batt,
				G.elapsed_time, 1900 + lt->tm_year, 1 + lt->tm_mon, lt->tm_mday,
				lt->tm_hour, lt->tm_min);
	}
	...
	if (G.is_berlin)
	{
		snprintf(buffer, sizeof(buffer) - 1, " ][%3d/%3d/%4d ", G.numaps,
				G.maxnumaps, G.maxaps);
	}
	...
	if (G.show_ap)
	{
		...
		if (G.show_manufacturer && (ws_col > (columns_ap - 4)))
		{
			...
			snprintf(strbuf + columns_ap + G.maxsize_essid_seen - 5, 15, "%s",
					"  MANUFACTURER");
		}
		...
		while (ap_cur != NULL)
		{
			...
			if (G.singlechan)
			{
				snprintf(strbuf + len, sizeof(strbuf) - len,
						"  %3d %3d %8ld %8ld %4d", ap_cur->avg_power,
						ap_cur->rx_quality, ap_cur->nb_bcn, ap_cur->nb_data,
						ap_cur->nb_dataps);
			}
			else
			{
				snprintf(strbuf + len, sizeof(strbuf) - len,
						"  %3d %8ld %8ld %4d", ap_cur->avg_power,
						ap_cur->nb_bcn, ap_cur->nb_data, ap_cur->nb_dataps);
			}
			...
			if (G.show_uptime)
			{
				snprintf(strbuf + len, sizeof(strbuf) - len, " %14s",
			...
			if (ws_col > (columns_ap - 4))
			{
				...
				{
					snprintf(strbuf, sizeof(strbuf) - 1, "%s", ap_cur->essid);
				}
				else
				{
					snprintf(strbuf, sizeof(strbuf) - 1, "<length:%3d>%s",
							ap_cur->ssid_length, "\x00");
				}
				...
				if (G.show_manufacturer)
				{
					...
					if (G.maxsize_essid_seen <= strlen(strbuf))
						...
						// write spaces (32)
						memset(strbuf + strlen(strbuf), 32,
								(G.maxsize_essid_seen - strlen(strbuf)));

					if (ap_cur->manuf == NULL)
						ap_cur->manuf = get_manufacturer(ap_cur->bssid[0],
								ap_cur->bssid[1], ap_cur->bssid[2]);

					snprintf(strbuf + G.maxsize_essid_seen,
							sizeof(strbuf) - G.maxsize_essid_seen, "  %s",
							ap_cur->manuf);
				}
	...
	//no AP found that is under attack
	if((G.singlechan == 1) && (bAP2AttackFound == 0))
	{
		LogMsg("No AP under attack found, continuing hopping");
		//than continue hopping
		G.singlechan = 0;
	}
		...
		while (ap_cur != NULL)
		{
			...
			while (st_cur != NULL)
			{
				...
				if (CHECK_REALLY_DEAUTH(ap_cur))
				{
					...
					unsigned long tempLastDeauthTimeDelta_us = 1000000
						* (time_beg.tv_sec
								- st_cur->m_timeval_lastDeauth.tv_sec)
						+ (time_beg.tv_usec
								- st_cur->m_timeval_lastDeauth.tv_usec);
					if((tempLastDeauthTimeDelta_us > 8000000) || ap_cur->bWPAHS_completed)
					{
						if (G.channel[0] != ap_cur->channel)
						{
							if (wi_set_channel(wi[0], ap_cur->channel) == 0)
					...
					if(!ap_cur->m_bAirbaseStarted && ap_cur->bWPAHS_completed)
					{
						RestartAirbase(ap_cur);
						ap_cur->m_bAirbaseStarted = 1;
					}
	...
	if (G.show_ack)
	{
		...
		while (na_cur != NULL)
		{
			...
			fprintf( stderr, " %02X:%02X:%02X:%02X:%02X:%02X", na_cur->namac[0],
					na_cur->namac[1], na_cur->namac[2], na_cur->namac[3],
...
int dump_write_csv(void)
{
	...
	while (ap_cur != NULL)
	{
		...
		fprintf(G.f_txt, "%02X:%02X:%02X:%02X:%02X:%02X, ", ap_cur->bssid[0],
		...
		fprintf(G.f_txt, ", %3d, %8ld, %8ld, ", ap_cur->avg_power,
				ap_cur->nb_bcn, ap_cur->nb_data);

		fprintf(G.f_txt, "%3d.%3d.%3d.%3d, ", ap_cur->lanip[0],
				ap_cur->lanip[1], ap_cur->lanip[2], ap_cur->lanip[3]);
	...
	while (st_cur != NULL)
	{
		...
		fprintf(G.f_txt, "%02X:%02X:%02X:%02X:%02X:%02X, ", st_cur->stmac[0],
				st_cur->stmac[1], st_cur->stmac[2], st_cur->stmac[3],
		...
		fprintf(G.f_txt, "%3d, %8ld, ", st_cur->power, st_cur->nb_pkt);
		...
		for (i = 0, n = 0; i < NB_PRB; i++)
		{
			...
			snprintf(ssid_list + n, sizeof(ssid_list) - n - 1, "%c",
					(i > 0) ? ',' : ' ');
...
char *get_manufacturer(unsigned char mac0, unsigned char mac1,
		unsigned char mac2)
{
	...
	else
	{
		...
		if (fp != NULL)
		{
			...
			while (fgets(buffer, sizeof(buffer), fp) != NULL)
			{
				...
				if (sscanf(buffer, "%2c-%2c-%2c", a, b, c) == 3)
				{
					snprintf(temp, sizeof(temp), "%c%c:%c%c:%c%c", a[0], a[1],
							b[0], b[1], c[0], c[1]);
...
int dump_write_kismet_netxml(void)
{
	...
	KISMET_NETXML_HEADER_BEGIN, G.airodump_start_time,
	...
	while (ap_cur != NULL)
	{
		...
		if (is_filtered_essid(ap_cur->essid)
				|| ap_cur->nb_pkt
						< 2 /* XXX: Maybe this last check should be removed */)
		{
			ap_cur = ap_cur->next;
			continue;
		}
		...
		{
			if (ap_cur->security & STD_WPA2)
				fprintf(G.f_kis_xml, "WPA2 ");
			if (ap_cur->security & STD_WPA)
				fprintf(G.f_kis_xml, "WPA ");
			if (ap_cur->security & STD_WEP)
				fprintf(G.f_kis_xml, "WEP ");
			if (ap_cur->security & STD_OPN)
				fprintf(G.f_kis_xml, "OPN ");
		}
		...
		{
			if (ap_cur->security & ENC_CCMP)
				fprintf(G.f_kis_xml, "AES-CCM ");
			if (ap_cur->security & ENC_WRAP)
				fprintf(G.f_kis_xml, "WRAP ");
			if (ap_cur->security & ENC_TKIP)
				fprintf(G.f_kis_xml, "TKIP ");
			if (ap_cur->security & ENC_WEP104)
				fprintf(G.f_kis_xml, "WEP104 ");
			if (ap_cur->security & ENC_WEP40)
				fprintf(G.f_kis_xml, "WEP40 ");
		...
		while (st_cur != NULL)
		{
			...
			client_max_rate =
					(st_cur->rate_from > st_cur->rate_to) ?
							st_cur->rate_from : st_cur->rate_to;
		...
		if (G.usegpsd)
		{
			fprintf(G.f_kis_xml, "\t\t<gps-info>\n"
					...
					ap_cur->gps_loc_min[1], ap_cur->gps_loc_min[2],
					ap_cur->gps_loc_min[3], ap_cur->gps_loc_max[0],
					ap_cur->gps_loc_max[1], ap_cur->gps_loc_max[2],
					ap_cur->gps_loc_max[3], ap_cur->gps_loc_best[0],
					ap_cur->gps_loc_best[1], ap_cur->gps_loc_best[2],
					/* Can the "best" be considered as average??? */
					ap_cur->gps_loc_best[0], ap_cur->gps_loc_best[1],
...
//     struct tm *ltime;
	...
	while (ap_cur != NULL)
	{
		...
		//BSSID
		fprintf(G.f_kis, "%02X:%02X:%02X:%02X:%02X:%02X;", ap_cur->bssid[0],
				ap_cur->bssid[1], ap_cur->bssid[2], ap_cur->bssid[3],
				ap_cur->bssid[4], ap_cur->bssid[5]);
		...
		{
			if (ap_cur->security & ENC_CCMP)
				fprintf(G.f_kis, "AES-CCM,");
			if (ap_cur->security & ENC_WRAP)
				fprintf(G.f_kis, "WRAP,");
			if (ap_cur->security & ENC_TKIP)
				fprintf(G.f_kis, "TKIP,");
			if (ap_cur->security & ENC_WEP104)
				fprintf(G.f_kis, "WEP104,");
			if (ap_cur->security & ENC_WEP40)
				fprintf(G.f_kis, "WEP40,");
		...
		//IP
		fprintf(G.f_kis, "%d.%d.%d.%d;", ap_cur->lanip[0], ap_cur->lanip[1],
...
void gps_tracker(void)
{
	...
	if (is_json)
	{
		...
		if (is_json)
		{
			...
			if (strncmp(line, "{\"class\":\"DEVICES\",\"devices\":[]}", 32)
					== 0)
			{
				close(gpsd_sock);
				return;
...
{
	unsigned int i = 0, chan_cur = 0, chan_first = 0, chan_last = 0, chan_max =
			128, chan_remain = 0;
...
{
	unsigned int i = 0, freq_cur = 0, freq_first = 0, freq_last = 0, freq_max =
			10000, freq_remain = 0;
	...
	G.own_frequencies = (int*) malloc(
			sizeof(int) * (freq_max - freq_remain + 1));
	...
	free(optc);
	if (i == 1)
		return G.own_frequencies[0];   //exactly 1 frequency given
	if (i == 0)
...
int check_monitor(struct wif *wi[], int *fd_raw, int *fdh, int cards)
{
	...
	for (i = 0; i < cards; i++)
	{
		...
		if (monitor != 0)
		{
			memset(G.message, '\x00', sizeof(G.message));
			snprintf(G.message, sizeof(G.message),
					"][ %s reset to monitor mode", wi_get_ifname(wi[i]));
...
int check_channel(struct wif *wi[], int cards)
{
	...
	for (i = 0; i < cards; i++)
	{
		...
		if (G.ignore_negative_one == 1 && chan == -1)
			return 0;
		if (G.channel[i] != chan)
		{
			memset(G.message, '\x00', sizeof(G.message));
			snprintf(G.message, sizeof(G.message), "][ fixed channel %s: %d ",
					wi_get_ifname(wi[i]), chan);
...
int check_frequency(struct wif *wi[], int cards)
{
	...
	for (i = 0; i < cards; i++)
	{
		...
		{
			memset(G.message, '\x00', sizeof(G.message));
			snprintf(G.message, sizeof(G.message), "][ fixed frequency %s: %d ",
					wi_get_ifname(wi[i]), freq);
...
int main(int argc, char *argv[])
{
	...
	snprintf(G.keyout, 511, "keyout-%02d%02d-%02d%02d%02d.keys", lt->tm_mon + 1,
				...
				for (j = 0; j < num_opts; j++)
				{
					if (strcmp(argv[i] + 1, long_options[j].name) == 0)
					{
						...
						if (i > 1 && strcmp(argv[i - 1], "-") == 0)
						{
							//separated dashes?
							printf(
									"Notice: You specified \"%s %s\". Did you mean \"%s%s\" instead?\n",
						...
						else
						{
							//forgot second dash?
							printf(
									"Notice: You specified \"%s\". Did you mean \"-%s\" instead?\n",
									argv[i], argv[i]);
						}
						break;
					}
	...
	do
	{
		...
		case 'i':
			...
			{
				printf(usage,
						getVersion("Airodump-ng", _MAJ, _MIN, _SUB_MIN,
								_REVISION, _BETA, _RC));
				fprintf(stderr,
...
#else
			printf(
					"Error: Airodump-ng wasn't compiled with pcre support; aborting\n");
#endif
			...
			while (output_format_string != NULL)
			{
				if (strlen(output_format_string) != 0)
				{
							...
							|| strncasecmp(output_format_string, "cap", 3) == 0)
					{
						...
						{
							printf(usage,
									getVersion("Airodump-ng", _MAJ, _MIN,
											_SUB_MIN, _REVISION, _BETA, _RC));
							fprintf(stderr,
									"Invalid output format: IVS and PCAP format cannot be used together.\n");
							return (1);
						}
					...
					else if (strncasecmp(output_format_string, "gps", 3) == 0)
					{
						G.usegpsd = 1;
							...
							|| strncasecmp(output_format_string, "newcore", 7)
									== 0
							|| strncasecmp(output_format_string, "kismet-nc", 9)
									== 0
							|| strncasecmp(output_format_string, "kismet_nc", 9)
									== 0
		...
		case 'H':
			...
			printf(usage,
					getVersion("Airodump-ng", _MAJ, _MIN, _SUB_MIN, _REVISION,
							_BETA, _RC));
			return (1);
...
//             printf("finished rearranging\n");
		...
		else    //use channels
		{
			...
			if (G.channel[0] == 0)
			{
				memset(&bg_chans_switch_times, 0, sizeof(bg_chans_switch_times));

#if 0 //remove original airodump channel hopper
				unused = pipe(G.ch_pipe);
				unused = pipe(G.cd_pipe);
	...
	G.airodump_start_time = (char *) realloc(G.airodump_start_time,
			...
			for (i = 0; i < G.num_cards; i++)
			{
				if ( FD_ISSET( fd_raw[i], &rfds ))
				{
							...
							== -1)
					{
						...
						memset(G.message, '\x00', sizeof(G.message));
						snprintf(G.message, sizeof(G.message),
								"][ interface %s down ", wi_get_ifname(wi[i]));
...
//                         return 1;
	...
	if (G.record_data)
	{
		...
		if (G.output_format_csv || G.f_txt != NULL)
			fclose(G.f_txt);
		if (G.output_format_kismet_csv || G.f_kis != NULL)
			fclose(G.f_kis);
